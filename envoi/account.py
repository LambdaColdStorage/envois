...
class Account(Contextable):
    ...
    """
    def __init__(self, bank='', name='', number='', swift='', address='', **kwargs):
        self.bank = bank
        self.name = name
        self.number = number
        self.swift = swift
    ...
    def __str__(self):
        ...
        """ % (self.name, self.number, self.swift, self.bank, self.address)

    def context(self):
        """
        self -> { variables to be included in template }
        """
        non_object_kvs = [(k, getattr(self, k, '')) for k in ['bank', 'name', 'number', 'swift']]
